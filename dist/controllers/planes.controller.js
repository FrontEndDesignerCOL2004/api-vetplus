...
exports.eliminarPlan = exports.actualizarPlan = exports.crearPlan = exports.obtenerPlanesPorTipo = exports.obtenerPlanPorId = exports.obtenerPlanes = void 0;
...
const obtenerPlanesPorTipo = async (req, res) => {
    try {
        ...
        const planes = await Plan_1.default.find({ type: tipo }); //En el modelo la propiedad es 'Type'
        res.json(planes);
    }
    catch (err) {
        res.status(500).json({ err: "Error al obtener planes por tipo" });
    }
};
...
const crearPlan = async (req, res) => {
    try {
        const { type, name, benefits, price } = req.body;
        const nuevoPlan = new Plan_1.default({ type, name, benefits, price });
        ...
        // res.status(400).json({ error: error.message, detalles: error.errors });
...
const actualizarPlan = async (req, res) => {
    try {
        ...
        const allowed = ["type", "name", "benefits", "price"];
        const updates = {};
        ...
        if (error.name === "ValidationError") {
            const mensajes = Object.values(error).map((err) => err.message);
